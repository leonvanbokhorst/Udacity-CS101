...
def calculate_pattern(patternnum):
    result = {}
    searchpattern = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']

    # init pattern
    for item in searchpattern:
        result[item] = '.'

    # calculate where the x's go and change them in the result
    for i in range(7, -1, -1):
        if patternnum == 0:
            break
        elif patternnum - 2 ** i >= 0:
            result[searchpattern[i]] = 'x'
            patternnum -= 2 ** i

    return result


def calculate_next_char(i, prev, pattern):
    comparestr = ''

    for j in range(3):
        comparestr += prev[(j + i - 1) % len(prev)]

    return pattern[comparestr]


def cellular_automaton(inputstr, patternnum, gennum):
    assert len(inputstr) > 0
    assert gennum >= 0
    assert 0 <= patternnum <= 255

    prev = inputstr
    row = ''
    rowlen = len(inputstr)
    pattern = calculate_pattern(patternnum)
    generation = 0

    while generation < gennum:
        row = ''
        for i in range(rowlen):
            row += calculate_next_char(i, prev, pattern)
...
# tests calculate_pattern
print calculate_pattern(1)
print calculate_pattern(0)
print calculate_pattern(40)
