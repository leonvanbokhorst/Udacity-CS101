...
def quick_sort(unsortedlist):
    ...
    if len(unsortedlist) <= 1:  # is already sorted
        return unsortedlist
    else:
        pivot = unsortedlist[0]  # pick pivot
        for element in unsortedlist[1:]:
            curval = element[1]
...
def ordered_search(index, ranks, keyword):
    ...
    if keyword in index:
        for url in index[keyword]:
            sortresult.append([url, ranks[url]])

    sortresult = quick_sort(sortresult)
